 ...
 * @author TheOnlyTails
 */
class opal : Item(opalTabProperty) {
	override fun interactLivingEntity(
		...
		if (target is SheepEntity && target.isAlive && !target.isSheared && target !is opalSheepEntity) {
			if (!playerIn.level.isClientSide) {
				EntityTypeRegistry.opalSheep.create(playerIn.level)?.apply {
					moveTo(
						target.x,
						target.y,
						target.z,
